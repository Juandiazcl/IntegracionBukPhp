...
FROM ((TARJASBUK2 AS T LEFT JOIN PERSONAL AS PE ON PE.rut_per=T.rut_per) LEFT JOIN cc22 AS lab ON (lab.cc4=t.cc4) AND (lab.cc3=t.cc3) AND (lab.cc2=t.cc2)) LEFT JOIN tratos11 AS tr ON (tr.codtrt=t.codtrt) AND (tr.cattrt=t.cattrt) AND (tr.cc1trt=t.cc1trt)
...
{
	if ($trabajador!=0){
		$sqlExiste="select rut_per, id_buk2 from personal where rut_per=".$trabajador;
	...
	while ($salida==0){
		$j=$j+1;
		if ($arrayCen[$j]['cc3']=='12' && $arrayCen[$j]['cc4']=='03') {
			$salida=1;
			$siTiene=1;
		}
	...
	if ($siTiene==1){
		echo "  Tiene ausencias  ";
		//extracionAusencias(2022, $mes, $trabajador);
	} else {
		echo "  NO Tiene ausencias  ";
	}
	$salida=0;
	$siTiene=0;
	$j=-1; 
	while ($salida==0){
		$j=$j+1;
		if ($arrayCen[$j]['cc3']=='12' && $arrayCen[$j]['cc4']=='02') {
			$salida=1;
			$siTiene=1;
		}
		if (count($arrayCen)==$j+1) {
	...
	if ($siTiene==1){
		echo "  Tiene vacaciones  ";
		
	//extraerVacaciones(2022, $mes, $trabajador);
	//envioVacaciones(2022, $mes, $trabajador);	
	} else {
...
function ExtraerLaboresNvas($An, $mes, $campo){
	$lab=0;
	$sqlNLab ="select distinct codigo_labor_buk as cLb from tarjasbuk2 as Tb
...
function extraerLicencias($An, $mes, $trabaja){  
   ...
   $sqlLic="select cc3, cc4, round(rut_per,0) as rut_perf, fechatj as fechatjf,  MONTH(FORMAT(fechatj,'dd/MM/yyyy')) as mes from tarjasbuk2 where YEAR(fechatj)=$An and MONTH(fechatj)=$mes".$filtroTrab." and cc3='12' and cc4='02'order by rut_per, fechatj";
   ...
   for($i=0;$i<count($arrayL);$i++){
         $conDias=$conDias+1;
  	 	if ($arrayL[$i]['rut_perf']!=$rutTemp){
 			...
 			echo " ----- CONSULTA ---";
 	 	    echo "UPDATE TARJASBUK2 SET fec_fLic=#".$varFecF."#, diaslic=$conDias WHERE rut_per=$varRut and fechatj=#".$priFec."#";
 	 	    echo " ----- ";
 	 	    executeSql("UPDATE TARJASBUK2 SET fec_fLic=#".$varFecF."#, diaslic=$conDias WHERE rut_per=$varRut and fechatj=#".$priFec."#");
...
//   }
  ...
  $sqlLicJs="select t.cc3, t.cc4, round(t.rut_per,0) as rut_perf, t.fechatj as fechatjf, t.diasLic, t.fec_fLic, tt.id_buk from tarjasbuk2 as t
  ...
  $arrayLj=executeSelect($sqlLicJs);
 	...
 	'justification' => "licencia valida",
   	...
   	$jsonGuideRaw = bsalePOST('https://remuneracionagricola.buk.cl/api/v1/chile/absences/licence', 'tj5UTcsKTnP63CWbTJFP2bgS', $data2L);
   	echo $data2L;
   }
}
...
// ************* For para sacar vacaciones todo 2022 count($arrayL) FORMAT(fechatj,'dd/MM/yyyy') as fechatjf
 function extraerVacaciones($An, $mes, $trabaja){ 
    ...
    $sqlLic="select cc3, cc4, round(rut_per,0) as rut_perf, fechatj as fechatjf, diasLic, fec_fLic from tarjasbuk2 where YEAR(fechatj)=$An and MONTH(fechatj)=$mes".$filtroTrab." and cc3='12' and cc4='01' order by rut_per, fechatj";
    $arrayL=executeSelect($sqlLic);
    $priFec=$arrayL[0]['fechatjf'];
    $rutTemp=$arrayL[0]['rut_perf'];
//    //$mesP=$arrayL[0]['mes'];
          ...
          $conDias=$conDias+1;
   	 	if ($arrayL[$i]['rut_perf']!=$rutTemp){
  			...
  			// echo " ----- CONSULTA ---";
  	 	    ...
  	 	    // echo " ----- ";
  	 	   executeSql("UPDATE TARJASBUK2 SET fec_fVac=#".$varFecF."#, diasVac=$conDias WHERE rut_per=$varRut and fechatj=#".$priFec."#");
  		$priFec=$arrayL[$i]['fechatjf'];
  		$rutTemp=$arrayL[$i]['rut_perf'];
  		$conDias=0;
		// echo " --- Rut - ";
...
function envioVacaciones($An, $mes){
    // {
 	...
 	// "percent_day": 0
//   }
   $sqlVacJs="select t.cc3, t.cc4, t.rut_per, t.fechatj as fechatjf, t.fec_fVac, tt.id_buk2 from tarjasbuk2 as t
   LEFT JOIN PERSONAL AS TT
   ON  TT.rut_per=T.rut_per  
     ...
     $arrayLj=executeSelect($sqlVacJs);
       for($i=0;$i<count($arrayLj);$i++){
    	...
    	$json2L = array(
     	...
     	$data2L =  json_encode($json2L);												
 	    $jsonGuideRaw = bsalePOST('https://remuneracionagricola.buk.cl/api/v1/chile/vacations', 'tj5UTcsKTnP63CWbTJFP2bgS', $data2L);
     	echo $data2L;
     }
  }
...
where lb.id=0";
 $array4 = executeSelect($sql4);
    for($i=0;$i<2;$i++){
  	 ...
  	 $jsonGuideRaw = bsalePOST('https://remuneracionagricola.buk.cl/api/v1/chile/piecework/tasks', 'tj5UTcsKTnP63CWbTJFP2bgS', $data4);
 	 echo $data4;
   }
...
$array3 = executeSelect($sql3);
    ...
    //  echo count($array3);
 	...
 	//json_encode($json3);
  	echo $data3;
   $jsonGuideRaw = bsalePOST('https://remuneracionagricola.buk.cl/api/v1/chile/employees', 'tj5UTcsKTnP63CWbTJFP2bgS', $data3);
 	}
...
//	
 	...
 	//json_encode($json3b);
  	echo $data3;
   $jsonGuideRaw = bsalePOST($linkBu, 'tj5UTcsKTnP63CWbTJFP2bgS', $data3);
 	}
}
...
//   	}
 	...
 	$jsonGuideRaw = bsalePOST('https://remuneracionagricola.buk.cl/api/v1/chile/piecework/executions', 'tj5UTcsKTnP63CWbTJFP2bgS', $data2);
	echo $data2;
  }
}
...
FROM (((TARJASBUK2 AS T LEFT JOIN PERSONAL AS PE ON PE.rut_per=T.rut_per) LEFT JOIN LaboresBuk AS Lb ON Lb.code=T.codigo_labor_buk) LEFT JOIN TarifasBuk AS Tb ON Tb.codigo_labor=Lb.id)
...
// 	//echo $json;
	...
	$jsonGuideRaw = bsalePOST('https://remuneracionagricola.buk.cl/api/v1/chile/piecework/worklogs', 'tj5UTcsKTnP63CWbTJFP2bgS', $data);	
 	echo $data;
 }
}
...
// ************ Extraccion de inasistencias OK  ********* 
 function extracionAusencias($An, $mes, $trabaja){ 
//{
...
//  echo count($sqlInasistencia);
 $sqlInasistencia="SELECT t.jornadatj, FORMAT(t.fechatj,'yyyy-mm-dd') as fechatjf , tt.id_buk2 as idbuk, t.rut_per, t.cc3, t.cc4 FROM TARJASBUK2 as t 
 LEFT JOIN PERSONAL AS TT
 ON  TT.rut_per=T.rut_per 
   ...
   echo $sqlInasistencia;
       for($i=0;$i<1;$i++){
    	...
    	'start_date' => $arrayIna[$i]['fechatjf'],
     	...
     	$jsonGuideRaw = bsalePOST('https://remuneracionagricola.buk.cl/api/v1/chile/absences/absence', 'tj5UTcsKTnP63CWbTJFP2bgS', $dataIna);
     	echo $dataIna;
     } 
}
...
function getIdEmpleado(){
	$jsonGuide = json_decode(bsaleGET('https://remuneracionagricola.buk.cl/api/v1/chile/employees?page_size=10000', 'tj5UTcsKTnP63CWbTJFP2bgS'), true);
	...
	function getIdtarifas(){
		$jsonGuideT = json_decode(bsaleGET('https://remuneracionagricola.buk.cl/api/v1/chile/piecework/executions?page_size=10000', 'tj5UTcsKTnP63CWbTJFP2bgS'), true);
...
function getIdLabores(){
	$jsonGuideL = json_decode(bsaleGET('https://remuneracionagricola.buk.cl/api/v1/chile/piecework/tasks?page_size=10000', 'tj5UTcsKTnP63CWbTJFP2bgS'), true);
...
function getIdLugares(){
	...
	$jsonGuideLu = json_decode(bsaleGET('https://remuneracionagricola.buk.cl/api/v1/chile/piecework/places?page_size=10000', 'tj5UTcsKTnP63CWbTJFP2bgS'), true);
