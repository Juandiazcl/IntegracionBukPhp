...
<head>
	...
	var listPersonal = '', listAnalysisUnit = '', listTipoTra = '';
	...
	if(userProfile=='ADM'){
		...
		$(document).ready(function() {
			...
			$('#txtRUT').focusout(function(event) {
				...
				if(verifyRUT($('#txtRUT').val())==true){
					...
					$.post('../../phps/personalb_Load.php', {type: 'verifyPersonal', id: id, rut: $('#txtRUT').val()}, function(data, textStatus, xhr) {
						if(data!=0){
					...
					$('#tableTallyBody > tr').each(function() {
						...
						if(count==total){
    						...
    						if(messageList!='' && labInd>2){
    							$("#modalList-text").html("Hay Asistencias erróneas o incompletas:" + messageList);
		...
		function loadLabour(){
			$.post('../../phps/labourBUK_Load.php', {
				...
				listLabour += '<option value="0" data-producto="A">AUSENCIA</option>';
				listLabour += '<option value="1" data-producto="L">LICENCIA</option>';
				listLabour += '<option value="2" data-producto="V">VACACIONES</option>';
				...
				if(data!=0){
					...
					for(i=0;i<data.length;i++){
						...
						var row = '<tr>' +
									...
									'<td style="width: 2%; overflow: hidden;">'+data[i]["codtj"]+'</td>' +
									...
									'<td style="width: 35%; overflow: hidden;"><select id="listLabour'+rowNumber+'" class="focused" data-live-search="true" data-width="fit" data-size="5" data-style="btn btn-default btn-xs" data-container="body" onChange="cambiarProductoUnidad(this)">'+listLabour+'</select></td>' +
									'<td style="width: 8%; overflow: hidden;"><input class="form-control input-sm" style="text-align: right;"   value="'+data[i]["valtjF"]+'" disabled></input></td>' +
									...
									'<td style="width: 5%; overflow: hidden;"><input class="form-control input-sm numbersOnlyFloat2" style="text-align: right;" onfocusout="verifyValue(this,this.value,1,0,0)" value="'+data[i]["jornadatj"]+'"></input></td>' +
									'<td style="width: 5%; overflow: hidden;"><input class="form-control input-sm numbersOnlyFloat2" style="text-align: right;" onfocusout="verifyValue(this,this.value,50000,0,0)" value="'+data[i]["rendtj"]+'"></input></td>' +
									'<td style="width: 5%; overflow: hidden;"><input class="form-control input-sm numbersOnlyFloat2" style="text-align: right;" onfocusout="verifyValue(this,this.value,'+maxExtraHours+',0,1)" value="'+data[i]["hhtj"]+'"></input></td>' +
									'<td style="width: 1%; overflow: hidden;"><input class="form-control input-sm numbersOnlyFloat2 focused" style="text-align: right;" value="'+data[i]["siTj"]+'" disabled></input></td>' +
								'</tr>';
						...
						$("#listTipoTra"+rowNumber).selectpicker();
						$("#listLabour"+rowNumber).val(data[i]["idTarF"]);
						$("#listLabour"+rowNumber).selectpicker();
	...
	<div class="container-fluid">
		<div class="row">
			<div class="panel panel-primary">
				<div class="panel-heading"><i class="fa fa-vcard-o fa-lg"></i>&nbsp;&nbsp; Tarjas por Trabajador Buk</div>
		        		...
		        		<div class="panel panel-primary">
							...
							<div class="panel-body">
								<div class="row">
									...
									<div class="col-xs-12 col-sm-12 col-md-2 col-lg-2">
										<!--<button id="btnEnviarHrs" class="btn btn-success btn-sm"><i class="fa fa-plus fa-lg fa-fw"></i>&nbsp;&nbsp;Enviar Horas Extras</button>-->
										...
										<table id="tableTallyHeader" class="table table-hover" style="font-size: 12px; width: 100%; table-layout: fixed;">
											<thead>
												...
												<tr>
													...
													<th style="width: 25%;">Larbor / Lugar</th>
													<th style="width: 10%;">Tarifa</th>
													<th class="warning" style="width: 10%; text-align: center;">Producto</th>
													<th class="warning" style="width: 10%; text-align: center;">Unidad</th>
													<th style="width: 6%; text-align: center;">Asis.</th>
