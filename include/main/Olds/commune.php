...
}elseif($_SESSION['display']['commune']['view']!=''){
	header('Location: ../../login.php');
}
	...
	var idList = 1;
		...
		$(document).ready(function() {
			...
			$("#delete").click(function() {
				...
				$.post('../../phps/commune_Save.php', {type: 'delete', id: id}, function(data, textStatus, xhr) {
					if(data=='OK'){
						loadRegistros();
						$("#modalDelete").modal('hide');
						...
						$("#modalDelete").modal('hide');
						$("#modal-text").text("No puede eliminar una comuna asociada a un sector/persona");
						$("#modal").modal('show');
					}
				});
			});
			...
			$("#save").click(function() {
				...
				$.post('../../phps/commune_Save.php', {type: type, id: $('#labelID').text(), name: $('#txtName').val().toUpperCase(), region: $('#listRegion').val()}, function(data, textStatus, xhr) {
					...
					if(data=='OK'){
						...
						$("#modal-text").text("Datos de comuna duplicados");
						$("#modal").modal('show');
					}
				});
			});
		...
		function loadRegistros(){
			//$("#tablaRegistros").html('<tr><th>ID</th><th>Region</th><th>Nombre</th><th>Editar</th><th>Eliminar</th></tr>');
		...
		function editRow(id){
			...
			$("#labelID").text(id);
			$("#modalNew").modal('show');
			$.post('../../phps/commune_Load.php', {type: "one", id: id}, function(data, textStatus, xhr) {
				var data = JSON.parse(data);
	...
	<div class="container">
		<div class="row">
			<div class="panel panel-redto">
				<div class="panel-heading"><i class="fa fa-map-signs fa-lg fa-fw"></i>&nbsp;&nbsp; Comunas</div>
				<div class="panel-body">
					<button id="new" class="btn btn-redto" <?php echo $_SESSION["display"]["commune"]["insert"]; ?>><span class="glyphicon glyphicon-plus-sign" aria-hidden="true"></span>&nbsp;&nbsp; Ingresar Nuevo</button></td>			
					<br/>
					<br/>
