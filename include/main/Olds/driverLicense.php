...
}elseif($_SESSION['display']['driverLicense']['view']!=''){
	header('Location: ../../login.php');
}
	...
	var idList = 1;
		...
		$(document).ready(function() {
			...
			$("#delete").click(function() {
				...
				$.post('../../phps/driverLicense_Save.php', {type: 'delete', id: id}, function(data, textStatus, xhr) {
					if(data=='OK'){
						loadRegistros();
						$("#modalDelete").modal('hide');
						...
						$("#modalDelete").modal('hide');
						$("#modal-text").text("No puede eliminar una licencia de conducir asociada a persona");
						$("#modal").modal('show');
					}
				});
			});
			...
			$("#save").click(function() {
				...
				$.post('../../phps/driverLicense_Save.php', {type: type, id: $('#labelID').text(), name: $('#txtName').val().toUpperCase()}, function(data, textStatus, xhr) {
					...
					if(data=='OK'){
						...
						$("#modal-text").text("Datos de Licencia duplicados");
						$("#modal").modal('show');
					}
				});
			});
		...
		function editRow(id){
			...
			$("#labelID").text(id);
			$("#modalNew").modal('show');
			$.post('../../phps/driverLicense_Load.php', {type: "one", id: id}, function(data, textStatus, xhr) {
				var data = JSON.parse(data);
	...
	<div class="container">
		<div class="row">
			<div class="panel panel-redto">
				<div class="panel-heading"><i class="fa fa-drivers-license-o fa-lg fa-fw"></i>&nbsp;&nbsp; Licencia de Conducir</div>
				<div class="panel-body">
					<button id="new" class="btn btn-redto" <?php echo $_SESSION["display"]["driverLicense"]["insert"]; ?>><span class="glyphicon glyphicon-plus-sign" aria-hidden="true"></span>&nbsp;&nbsp; Ingresar Nuevo</button></td>			
					<br/>
					<br/>
