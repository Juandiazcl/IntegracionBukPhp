...
}elseif($_SESSION['display']['region']['view']!=''){
	header('Location: ../../login.php');
}
	...
	var idList = 1;
		...
		$(document).ready(function() {
			...
			$("#delete").click(function() {
				...
				$.post('../../phps/region_Save.php', {type: 'delete', id: id}, function(data, textStatus, xhr) {
					if(data=='OK'){
						loadRegistros();
						$("#modalDelete").modal('hide');
						...
						$("#modalDelete").modal('hide');
						$("#modal-text").text("No puede eliminar una región que posea comunas");
						$("#modal").modal('show');
					}
				});
			});
			...
			$("#save").click(function() {
				...
				$.post('../../phps/region_Save.php', {type: type, id: $('#labelID').text(), name: $('#txtName').val().toUpperCase(), number: $('#txtNumber').val().toUpperCase()}, function(data, textStatus, xhr) {
					...
					if(data=='OK'){
						...
						$("#modal-text").text("Datos de región duplicados");
						$("#modal").modal('show');
					}
				});
			});
		...
		function loadRegistros(){
			//$("#tablaRegistros").html('<tr><th>ID</th><th>Número</th><th>Nombre</th><th>Editar</th><th>Eliminar</th></tr>');
			$("#tablaRegistros").html('<thead><tr>' +
				'<th data-dynatable-column="id">ID</th>' +
				'<th data-dynatable-column="number">Número</th>' +
		...
		function editRow(id){
			...
			$("#labelID").text(id);
			$("#modalNew").modal('show');
			$.post('../../phps/region_Load.php', {type: "one", id: id}, function(data, textStatus, xhr) {
				var data = JSON.parse(data);
	...
	<div class="container">
		<div class="row">
			<div class="panel panel-redto">
				<div class="panel-heading"><i class="fa fa-map fa-lg fa-fw"></i>&nbsp;&nbsp; Regiones</div>
				<div class="panel-body">
					<button id="new" class="btn btn-redto" <?php echo $_SESSION["display"]["region"]["insert"]; ?>><span class="glyphicon glyphicon-plus-sign" aria-hidden="true"></span>&nbsp;&nbsp; Ingresar Nuevo</button></td>			
					<br/>
					<br/>
										...
										<div class="container-fluid">
											<div class="row">
												<div class="col-xs-12 col-sm-12 col-md-2 col-lg-2">
										  			<label>Número:</label>
					  								<input id="txtNumber" type="text" class="form-control" maxlength="3">	
												</div>
