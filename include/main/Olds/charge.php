...
}elseif($_SESSION['display']['charge']['view']!=''){
	header('Location: ../../login.php');
}
	...
	var idList = 1;
		...
		$(document).ready(function() {
			...
			$("#delete").click(function() {
				...
				$.post('../../phps/charge_Save.php', {type: 'delete', id: id}, function(data, textStatus, xhr) {
					if(data=='OK'){
						loadRegistros();
						$("#modalDelete").modal('hide');
						...
						$("#modalDelete").modal('hide');
						$("#modal-text").text("No puede eliminar un cargo asociado a persona");
						$("#modal").modal('show');
					}
				});
			});
			...
			$("#save").click(function() {
				...
				$.post('../../phps/charge_Save.php', {type: type, id: $('#labelID').text(), name: $('#txtName').val().toUpperCase()}, function(data, textStatus, xhr) {
					...
					if(data=='OK'){
						...
						$("#modal-text").text("Datos de especialidad duplicados");
						$("#modal").modal('show');
					}
				});
			});
		...
		function editRow(id){
			...
			$("#labelID").text(id);
			$("#modalNew").modal('show');
			$.post('../../phps/charge_Load.php', {type: "one", id: id}, function(data, textStatus, xhr) {
				var data = JSON.parse(data);
	...
	<div class="container">
		<div class="row">
			<div class="panel panel-redto">
				<div class="panel-heading"><i class="fa fa-map fa-lg fa-fw"></i>&nbsp;&nbsp; Especialidades</div>
				<div class="panel-body">
					<button id="new" class="btn btn-redto" <?php echo $_SESSION["display"]["charge"]["insert"]; ?>><span class="glyphicon glyphicon-plus-sign" aria-hidden="true"></span>&nbsp;&nbsp; Ingresar Nuevo</button></td>			
					<br/>
					<br/>
