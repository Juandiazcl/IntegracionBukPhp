...
}elseif($_SESSION['display']['sector']['view']!=''){
	header('Location: ../../login.php');
}
	...
	var idList = 1;
		...
		$(document).ready(function() {
			...
			$("#delete").click(function() {
				...
				$.post('../../phps/sector_Save.php', {type: 'delete', id: id}, function(data, textStatus, xhr) {
					if(data=='OK'){
						loadRegistros();
						$("#modalDelete").modal('hide');
						...
						$("#modalDelete").modal('hide');
						$("#modal-text").text("No puede eliminar un sector asociado a personal");
						$("#modal").modal('show');
					}
				});
			});
			...
			$("#save").click(function() {
				...
				$.post('../../phps/sector_Save.php', {type: type, id: $('#labelID').text(), name: $('#txtName').val().toUpperCase(), commune: $('#listCommune').val()}, function(data, textStatus, xhr) {
					...
					if(data=='OK'){
						...
						$("#modal-text").text("Datos de sector duplicados");
						$("#modal").modal('show');
					}
				});
			});
		...
		function loadRegistros(){
			//$("#tablaRegistros").html('<tr><th>ID</th><th>Region</th><th>Comuna</th><th>Nombre</th><th>Editar</th><th>Eliminar</th></tr>');
			$("#tablaRegistros").html('<thead><tr>' +
				'<th data-dynatable-column="id">ID</th>' +
				'<th data-dynatable-column="region_name">Región</th>' +
			...
			$.post('../../phps/sector_Load.php', {type: "all"}, function(data, textStatus, xhr) {
				if(data!=0){
					...
					for(i=0;i<data.length;i++){
						...
						list += "<td>"+data[i]['number']+" - "+data[i]['region_name']+"</td>";
						list += "<td>"+data[i]['commune_name']+"</td>";
		...
		function editRow(id){
			...
			$("#labelID").text(id);
			$("#modalNew").modal('show');
			$.post('../../phps/sector_Load.php', {type: "one", id: id}, function(data, textStatus, xhr) {
				var data = JSON.parse(data);
	...
	<div class="container">
		<div class="row">
			<div class="panel panel-redto">
				<div class="panel-heading"><i class="fa fa-map-marker fa-lg fa-fw"></i>&nbsp;&nbsp; Sectores</div>
				<div class="panel-body">
					<button id="new" class="btn btn-redto" <?php echo $_SESSION["display"]["sector"]["insert"]; ?>><span class="glyphicon glyphicon-plus-sign" aria-hidden="true"></span>&nbsp;&nbsp; Ingresar Nuevo</button></td>			
					<br/>
					<br/>
