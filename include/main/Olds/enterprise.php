...
}elseif($_SESSION['display']['enterprise']['view']!=''){
	header('Location: ../../login.php');
}
...
<head>
	...
	var idList = 1;
		...
		$(document).ready(function() {
			...
			$('#txtRUT').focusout(function(event) {
				...
				if(verifyRUT($('#txtRUT').val())==true){
					...
					$.post('../../phps/enterprise_Load.php', {type: 'verify', id: id, rut: $('#txtRUT').val(), filter: 1}, function(data, textStatus, xhr) {
						if(data!=0){
			...
			$("#delete").click(function() {
				...
				$.post('../../phps/enterprise_Save.php', {type: 'delete', id: id}, function(data, textStatus, xhr) {
					if(data=='OK'){
						...
						$("#modalDelete").modal('hide');
						$("#modal-text").text("No puede eliminar un empleador asociado a contrato");
						$("#modal").modal('show');
					}
				});
			});
					...
					$.post('../../phps/enterprise_Load.php', {type: 'verify', id: id, rut: $('#txtRUT').val(), filter: 1}, function(data, textStatus, xhr) {
						if(data!=0){
							...
							$.post('../../phps/enterprise_Save.php', {type: type, 
								...
								legal_represent_name: $('#txtRepresentName').val().toUpperCase(),
								legal_represent_commune_id: $('#listRepresentCommune').val(),
								legal_represent_address: $('#txtRepresentAddress').val().toUpperCase(),
								legal_represent_city: $('#txtRepresentCity').val().toUpperCase()}, function(data, textStatus, xhr) {
								$("#modalProgress").modal('hide');
								console.log(data);
								if(data=='OK'){
									loadRegistros();
									...
									$("#modal-text").text("Datos de empleador duplicados");
									$("#modal").modal('show');
								}
							});			
						}
		...
		function loadRegistros(){
			...
			$.post('../../phps/enterprise_Load.php', {type: "all", filter: 1}, function(data, textStatus, xhr) {
				if(data!=0){
		...
		function editRow(id){
			...
			$.post('../../phps/enterprise_Load.php', {type: "one", id: id}, function(data, textStatus, xhr) {
				...
				$('#listRepresentRegion').val(data[0]['represent_region_id']);
				loadRepresentCommunes("selected",data[0]['legal_represent_commune_id'],0);
				$('#txtRepresentAddress').val(data[0]['legal_represent_address']);
				$('#txtRepresentCity').val(data[0]['legal_represent_city']);
		...
		function loadRepresentCommunes(type, id, idSector){
			if(type=="all"){
				$.post('../../phps/commune_Load.php', {type: "list", region_id: $("#listRepresentRegion").val()}, function(data, textStatus, xhr) {
					var data = JSON.parse(data);
					$("#listRepresentCommune").html('');
					for(i=0;i<data.length;i++){
						$("#listRepresentCommune").append('<option value="'+data[i]["id"]+'">'+data[i]["name"]+'</option>');
					}
				});
			}else{
				$.post('../../phps/commune_Load.php', {type: "list", region_id: $("#listRepresentRegion").val()}, function(data, textStatus, xhr) {
					...
					for(i=0;i<data.length;i++){
						...
						$("#listRepresentCommune").append('<option value="'+data[i]["id"]+'" '+selected+'>'+data[i]["name"]+'</option>');
					}
				});
			}
		}

		function loadRepresentRegions(){
			$.post('../../phps/region_Load.php', {type: "all"}, function(data, textStatus, xhr) {
				var data = JSON.parse(data);
				for(i=0;i<data.length;i++){
					$("#listRepresentRegion").append('<option value="'+data[i]["id"]+'">'+data[i]["number"]+' - '+data[i]["name"]+'</option>');
				}
				loadRepresentCommunes('all',0);
			});
			$("#listRepresentRegion").change(function(){
				loadRepresentCommunes('all',0);
			});
		}
		...
		function cleanModal(){
			...
			$('#txtRepresentAddress').val('');
			$('#txtRepresentCity').val('');
		}
	...
	<div class="container">
		<div class="row">
			<div class="panel panel-redto">
				<div class="panel-heading"><i class="fa fa-building fa-lg fa-fw"></i>&nbsp;&nbsp; Empleador</div>
				<div class="panel-body">
					<button id="new" class="btn btn-redto" <?php echo $_SESSION["display"]["enterprise"]["insert"]; ?>><span class="glyphicon glyphicon-plus-sign" aria-hidden="true"></span>&nbsp;&nbsp; Ingresar Nuevo</button></td>			
					<br/>
					<br/>
									...
									<div class="panel-body">
										<div class="container-fluid">
											<div class="row">
												...
												<div class="col-xs-12 col-sm-12 col-md-4 col-lg-4">
										  			<label>Región:</label>
					  								<select id="listRepresentRegion" class="form-control" style="width: 100%;"></select>
												</div>
												<div class="col-xs-12 col-sm-12 col-md-4 col-lg-4">
										  			<label>Comuna:</label>
					  								<select id="listRepresentCommune" class="form-control" style="width: 100%;"></select>
												</div>
												<div class="col-xs-12 col-sm-12 col-md-4 col-lg-4">
										  			<label>Ciudad:</label>
					  								<input id="txtRepresentCity" type="Name" class="form-control" maxlength="45" >	
												</div>
												<div class="col-xs-12 col-sm-12 col-md-8 col-lg-8">
										  			<label>Dirección:</label>
					  								<input id="txtRepresentAddress" type="Name" class="form-control mandatory" maxlength="100">
												</div>
